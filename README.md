...
- Reloc-aware `.text`/EP vs disk and COW overwrite run only for Windows builtin leaves, because third-party games/AC packers otherwise firehose `exe_text`. Empty `PsLoadedModuleList` is fail-closed so hook/pool scans do not treat every pointer as unbacked. The module-inventory baseline diff is fail-closed the same way: an unreadable inventory withholds `driver.vanished` / `driver.unnotified_load` / `driver.remap` and prints `scan_failed:modinv:*` once. Post-load hiding is covered by host-side diff only, because the kernel-side module-list fingerprint push is not implemented, and a non-inbox `.sys` that legitimately deletes its own service key can surface an informational `driver.evidence_asymmetry` (`reason=no_service_key`).
