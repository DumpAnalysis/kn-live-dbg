...
전체 페이지 순회 확장은 꼬리 페이지, 범위 갱신 시 커서 유지, PID 재사용, 큐 퇴출 우선순위, NX/U/S 상속, 큰 페이지 PAT/예약 비트, VA alias, PTE 읽기 실패, PE 권한 불일치와 소유권 미확정을 검사한다. 실제 게임, DKOM, 커널 드라이버 장치 수명주기, 세션별 win32k 매핑 및 장시간 오탐률은 사용자가 별도 시험 호스트에서 검증한다. 합성 시험과 자체 프로세스 시험 결과를 실제 게임핵 재현율로 해석하면 안 된다.
