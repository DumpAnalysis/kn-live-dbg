...
최종 수정본은 x64 Release·Debug 빌드와 아래 검사를 통과했다. 반복 코드 검토에서 발견한 문제를 수정하고 관련 회귀를 다시 실행한 뒤, 검토 범위에서 추가 조치가 필요한 문제는 찾지 못했다. 같은 구현 에이전트가 수행한 적대적 재검토이며 독립 분석가 감사는 아니다.
